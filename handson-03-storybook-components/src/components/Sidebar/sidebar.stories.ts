...
const meta: Meta<SidebarComponent> = {
  ...
  decorators: [
    (story) => ({
      ...story(),
      template: `<div class="h-[600px]">${story().template || '<fin-sidebar></fin-sidebar>'}</div>`,
    }),
  ],
};
...
export const Default: Story = {
  render: () => ({
    props: {
      ...
      active: 'dashboard',
      collapsed: false,
      onItemClick(key: string) { this['active'] = key; },
      onCollapsedChange(val: boolean) { this['collapsed'] = val; },
    },
    template: `
      <div class="h-[600px]">
        <fin-sidebar
